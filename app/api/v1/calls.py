...
NOTE ON BACKGROUND/KILLED-APP DELIVERY:
(b) and (c) above are BEST EFFORT. Neither can guarantee delivery to a fully
force-killed app — that requires native PushKit/CallKit (iOS) or a
foreground service + full-screen intent (Android), which in turn requires
wrapping this website in a native shell (e.g. Capacitor). See the
accompanying engineering note for the two paths and their tradeoffs.
"""
