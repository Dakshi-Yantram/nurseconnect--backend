...
async def otp_send(payload: OtpSendRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    """
    phone = _normalize_phone(payload.phone_e164)
    # Play Store review account: fixed OTP, no SMS, no send throttle.
    is_review_account = False
    if settings.REVIEW_TEST_PHONE and settings.REVIEW_TEST_OTP:
        try:
            is_review_account = phone == _normalize_phone(settings.REVIEW_TEST_PHONE)
        except Exception:
            logger.warning("REVIEW_TEST_PHONE is not a valid phone number; ignoring it.")
